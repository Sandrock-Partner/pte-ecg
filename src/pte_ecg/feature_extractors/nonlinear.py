...
class NonlinearExtractor(BaseFeatureExtractor):
    ...
    Examples:
        # Extract all nonlinear features (via FeatureExtractor)
    ...
    def __init__(self, parent: FeatureExtractor, n_jobs: int = -1):
        """Initialize the nonlinear extractor.
    ...
    def get_features(self, ecg: np.ndarray) -> pd.DataFrame:
        ...
        args_list = [(ecg_single, sfreq, lead_order) for ecg_single in ecg]
        processes = utils.get_n_processes(self.n_jobs, n_samples)

        if processes == 1:
            results = list(
                tqdm(
                    (self._process_single_sample(sample_data, sfreq, lead_order) for sample_data, _, _ in args_list),
                    total=n_samples,
    ...
    def _process_single_sample(self, sample_data: np.ndarray, sfreq: float, lead_order: list[str]) -> dict[str, float]:
        ...
        for ch_num, ch_data in enumerate(sample_data):
            ...
            features.update((f"nonlinear_{key}_{lead_name}", value) for key, value in ch_feat.items())
        return features
    ...
    def _process_single_sample_static(sample_data: np.ndarray, sfreq: float, lead_order: list[str]) -> dict[str, float]:
        ...
        features: dict[str, float] = {}
        for ch_num, ch_data in enumerate(sample_data):
            ch_feat = NonlinearExtractor._process_single_channel_static(ch_data, sfreq, ch_num)
            lead_name = lead_order[ch_num] if ch_num < len(lead_order) else f"ch{ch_num}"
            features.update((f"nonlinear_{key}_{lead_name}", value) for key, value in ch_feat.items())
        return features

    def _process_single_channel(self, ch_data: np.ndarray, sfreq: float, ch_num: int) -> dict[str, float]:
    ...
    @staticmethod
    def _process_single_channel_static(ch_data: np.ndarray, sfreq: float, ch_num: int) -> dict[str, float]:
        ...
        """
        if not HAS_NOLDS:
            raise ImportError("nolds is required for nonlinear features. Install with: pip install pte-ecg[nonlinear]")
        ...
        features["dfa_alpha1"] = nolds.dfa(ch_data, nvals=[4, 8, 16, 32]) if half_len > 32 else np.nan
        features["dfa_alpha2"] = nolds.dfa(ch_data, nvals=[64, 128, 256]) if half_len > 256 else np.nan
        features["change_dfa_alpha"] = (
            nolds.dfa(ch_data[:half_len], nvals=[4, 8, 16]) - nolds.dfa(ch_data[half_len:], nvals=[4, 8, 16])
            if half_len > 16
            else np.nan
        )
        ...
        try:
            ...
            logger.warning(f"Error calculating embedding dimension for channel {ch_num}: {e}")

        # Lyapunov exponent
        try:
            lyap_exp = features["largest_lyapunov_exponent"] = nolds.lyap_r(ch_data, emb_dim=embedding_dim)
        ...
        try:
            ...
            logger.warning(f"Error calculating correlation dimension for channel {ch_num}: {e}")
        ...
        features["recurrence_network_measures"] = (rqa["Determinism"].iat[0] + rqa["Laminarity"].iat[0]) / 2
        features["rqa_l_entropy"] = rqa["LEn"].iat[0]

        # Time irreversibility and nonlinear variance
        diffs = np.diff(ch_data)
        features["time_irreversibility"] = float(np.mean(diffs**3) / (np.mean(diffs**2) ** 1.5))
        ...
        windows = np.lib.stride_tricks.sliding_window_view(ch_data, window_size)[::step_size]
        local_vars = np.var(windows, axis=1)
        features["dynamic_variance"] = float(np.var(local_vars))

        # Multiscale entropy
        features["multiscale_entropy"], _ = nk.entropy_sample(ch_data, dimension=embedding_dim, scale=2)

        # Symbolic dynamics
        above_median = ch_data > np.median(ch_data)
        features["symbolic_dynamics"] = np.sum(np.abs(np.diff(above_median))) / (len(above_median) - 1)
        ...
        features["sample_entropy_change_rate"] = (
            (entropy2 - entropy1) / (entropy1 + utils.EPS) if entropy1 != 0 else np.nan
        )
        ...
        features["singular_spectrum_entropy"] = -np.sum(s_norm * np.log2(s_norm + utils.EPS))
        ...
        features["complexity_loss"] = (entropy_orig - entropy_filt) / entropy_orig if entropy_orig != 0 else np.nan
